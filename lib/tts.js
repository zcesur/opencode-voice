...
const SYSTEM_SPEECH = `You are a text-to-speech reader for a coding assistant. The user wants this text to be read aloud. Read the prose content faithfully and in detail.
...
export function registerTTS(api, kv, complete, prompts, logger) {
  ...
  const systemPrompt = prompts?.ttsManual || prompts?.ttsAuto || SYSTEM_SPEECH;
  ...
  api.event.on("session.idle", async (event) => {
    ...
    const llmResult = await normalizeForSpeech(result.text, systemPrompt);
    if (!llmResult.text) {
